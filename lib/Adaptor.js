 ...
 * @public
 * @example
 * normalizeCSVarray([csvStr],{ delimiter: ';', noheader: true });
 * @constructor
 * @param {array} csvStr - Array of strings
...
function normalizeCSVarray(csvStr, options, callback) {
  return state => {
    (0, _csvtojson.default)({
      noheader: true,
      output: 'csv'
    }).fromString(csvStr).then(csvRow => {
