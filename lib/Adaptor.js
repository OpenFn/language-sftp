...
function getCSV(filePath, encoding, parsingOptions) {
  return state => {
    ...
    const {
      filter
    } = parsingOptions; // Create Stream, Writable AND Readable

    let inoutStream = new _stream.Transform({
    ...
    return sftp.connect(state.configuration).then(() => {
      process.stdout.write('Connected. ✓\n');
      inoutStream = sftp.get(filePath); // return sftp.get(filePath);
      ...
      let readStream = new _stream.Readable({
        objectMode: true,

        read() {}

      });
      readStream.pipe(stream); // readStream.pipe(JSONStream.parse());
      // console.log(stream);

      return new Promise((resolve, reject) => {
        readStream.pipe((0, _csvParser.default)()).on('readable', jsonObject => {
          console.log(jsonObject); // while (null !== (jsonObject = stream.read())) {
          //   arr.push(jsonObject);
          // }
        }).on('data', row => {
          console.log(row);
        ...
        console.log('json', json); // const nextState = composeNextState(state, json);
